...
export class PlayerControls {
  constructor(scene, room, options = {}) {
    ...
    this.camera.position.set(this.playerX - 5, this.playerY + 2, this.playerZ); 
  ...
  initializeMobileControls() {
    ...
    // Modify jump button event listener to handle manual flip
    document.getElementById('jump-button').addEventListener('touchstart', (event) => {
      event.preventDefault();
      
      // If the vehicle is flipped, attempt to flip it back
      if (this.vehicle) {
        this.vehicle.manualFlip();
      } 
      // Otherwise, perform the original jump
    ...
    // Add acceleration and brake buttons
    const accButton = document.getElementById('acc-button');
    if (!accButton) {
      const newAccButton = document.createElement('div');
      newAccButton.id = 'acc-button';
      newAccButton.innerText = 'ACCELERATE';
      document.body.appendChild(newAccButton);
    }
    
    const brakeButton = document.getElementById('brake-button');
    if (!brakeButton) {
      const newBrakeButton = document.createElement('div');
      newBrakeButton.id = 'brake-button';
      newBrakeButton.innerText = 'BRAKE';
      document.body.appendChild(newBrakeButton);
    }
    
    accButton.pressed = false;
    brakeButton.pressed = false;
    
    accButton.addEventListener('touchstart', (e) => {
      e.preventDefault();
      accButton.pressed = true;
    });
    
    accButton.addEventListener('touchend', (e) => {
      e.preventDefault();
      accButton.pressed = false;
    });
    ...
    // Modify joystick event handling to handle start, move, and end states
    this.joystick.on('start', (evt, data) => {
      // Reset steering and movement values when joystick starts
      this.moveRight = 0;
    });
    ...
    this.joystick.on('move', (evt, data) => {
      ...
      // Only use horizontal axis for steering
      this.moveRight = Math.cos(angle) * force * SPEED * 8;    
    });

    this.joystick.on('end', () => {
      // Reset steering to zero when joystick is released
      this.moveRight = 0;
    });
  }
  ...
  setupEventListeners() {
    ...
    document.addEventListener("keydown", (e) => {
      ...
      // Handle jump with spacebar - now also works for manual vehicle flip
      if (e.key === " ") {
        if (this.vehicle) {
          this.vehicle.manualFlip();
        } else if (this.canJump) {
  ...
  processMovement() {
    ...
    // Use vehicle controls if available
    if (this.vehicle) {
      this.processVehicleMovement();
      return;
    }
    ...
    blockMeshes.forEach(block => {
      if (block.type === "Group" && block.userData.isTree) {
        this.checkCollision(block, 1.0, 2.0, 1.0); 
      } else {
        this.checkCollision(block);
      }
    });
    
    this.checkCollision = function(block, overrideWidth, overrideHeight, overrideDepth) {
  ...
  processVehicleMovement = function() {
    let forward = false;
    let backward = false;
    let left = false;
    let right = false;
    ...
    if (!this.isMobile) {
      forward = this.keysPressed.has("w") || this.keysPressed.has("arrowup");
      backward = this.keysPressed.has("s") || this.keysPressed.has("arrowdown");
      
      // Add A and D key steering control
      if (this.keysPressed.has("a") || this.keysPressed.has("arrowleft")) {
      ...
      if (this.moveRight > 0) right = true;
      if (this.moveRight < 0) left = true;
    }
    
    this.vehicle.applyControls(forward, backward, left, right, steering);
    ...
    if (this.controls) {
      ...
      const idealOffset = new THREE.Vector3(-5, 3, 0);
      
      const rotationMatrix = new THREE.Matrix4().makeRotationY(rotation);
      const transformedOffset = idealOffset.clone().applyMatrix4(rotationMatrix);

      this.camera.position.copy(targetPos).add(transformedOffset);
      this.camera.lookAt(targetPos);
    }
    
    if (this.room && (
        Math.abs(this.lastPosition.x - position.x) > 0.01 ||
        Math.abs(this.lastPosition.y - position.y) > 0.01 ||
        Math.abs(this.lastPosition.z - position.z) > 0.01 ||
